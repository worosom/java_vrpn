...
vrpn_TextPrinter::vrpn_TextPrinter() :
		d_first_watched_object(NULL), d_ostream(stdout), d_severity_to_print(
				vrpn_TEXT_WARNING), d_level_to_print(0) {
}
...
int vrpn_TextPrinter::add_object(vrpn_BaseClass *o) {
	...
	if (o->d_connection->register_handler(o->d_text_message_id,
			text_message_handler, victim, o->d_sender_id) != 0) {
		fprintf(stderr,
...
void vrpn_TextPrinter::remove_object(vrpn_BaseClass *o) {
	...
	while ((victim != NULL)
			&& ((o->d_connection != victim->obj->d_connection)
					|| (strcmp(o->d_servicename, victim->obj->d_servicename)
							!= 0))) {
	...
	if (victim != NULL) {
		...
		if (o->d_connection) {
			if (o->d_connection->unregister_handler(o->d_text_message_id,
					text_message_handler, victim, o->d_sender_id) != 0) {
				fprintf(stderr,
						"vrpn_TextPrinter::remove_object(): Can't unregister callback\n");
			}
		}
...
int vrpn_TextPrinter::text_message_handler(void *userdata,
		vrpn_HANDLERPARAM p) {
	...
	// Decode the message
	if (vrpn_BaseClassUnique::decode_text_message_from_buffer(message,
			&severity, &level, p.buffer) != 0) {
		fprintf(stderr,
...
int vrpn_BaseClass::init(void) {
	...
	if (d_connection) {
		d_text_message_id = d_connection->register_message_type(
		...
		d_ping_message_id = d_connection->register_message_type(
		...
		d_pong_message_id = d_connection->register_message_type(
...
vrpn_BaseClassUnique::vrpn_BaseClassUnique() :
		d_connection(NULL), d_servicename(NULL), d_num_autodeletions(0), d_first_mainloop(
				1), d_unanswered_ping(0), d_flatline(0) {
...
int vrpn_BaseClassUnique::encode_text_message_to_buffer(char * buf,
		vrpn_TEXT_SEVERITY severity, vrpn_uint32 level, const char * msg) {
	char *bufptr = buf;
...
int vrpn_BaseClassUnique::decode_text_message_from_buffer(char *msg,
		vrpn_TEXT_SEVERITY *severity, vrpn_uint32 *level, const char *buf) {
...
int vrpn_BaseClassUnique::send_text_message(const char *msg,
		struct timeval timestamp, vrpn_TEXT_SEVERITY type, vrpn_uint32 level) {
	...
	if (len > vrpn_MAX_TEXT_LEN) {
		fprintf(stderr,
				"vrpn_BaseClassUnique::send_message: Attempt to encode string that is too long\n");
		return -1;
	}
	...
	if (d_connection) {
		d_connection->pack_message(sizeof(buffer), timestamp, d_text_message_id,
				d_sender_id, buffer, vrpn_CONNECTION_RELIABLE);
	}
...
int vrpn_BaseClassUnique::handle_ping(void *userdata, vrpn_HANDLERPARAM) {
	...
	if (me->d_connection != NULL) {
		me->d_connection->pack_message(0, now, me->d_pong_message_id,
				me->d_sender_id, NULL, vrpn_CONNECTION_RELIABLE);
	}
