...
vrpn_File_Connection::vrpn_File_Connection(const char * station_name,
		...
		vrpn_Connection(local_in_logfile_name, local_out_logfile_name, NULL,
				NULL), d_controllerId(register_sender("vrpn File Controller")), d_set_replay_rate_type(
				register_message_type("vrpn_File set_replay_rate")), d_reset_type(
				register_message_type("vrpn_File reset")), d_play_to_time_type(
				register_message_type("vrpn_File play_to_time")), d_fileName(
				NULL), d_file(NULL), d_logHead(NULL), d_logTail(NULL), d_currentLogEntry(
				NULL), d_startEntry(NULL), d_preload(
				vrpn_FILE_CONNECTIONS_SHOULD_PRELOAD), d_accumulate(
				vrpn_FILE_CONNECTIONS_SHOULD_ACCUMULATE) {
	...
	if (d_endpoints[0] == NULL) {
		fprintf(stderr,
				"vrpn_File_Connection::vrpn_File_Connection(): NULL zeroeth endpoint\n");
	} else {
...
void vrpn_File_Connection::FileTime_Accumulator::accumulate_to(
	...
	= vrpn_TimevalSum(         // summed with previous elapsed time
			accum, vrpn_TimevalScale(      // scaled by replay rate
					vrpn_TimevalDiff(   // elapsed wallclock time
							now_time, d_time_of_last_accum), d_replay_rate));
...
int vrpn_File_Connection::mainloop(const timeval * /*timeout*/) {
	...
	const timeval end_time = vrpn_TimevalSum(d_time,
			d_filetime_accum.accumulated());
...
int vrpn_File_Connection::playone_to_filetime(timeval end_filetime) {
	...
	retval = endpoint->d_inLog->logIncomingMessage(header.payload_len, now,
			header.type, header.sender, header.buffer);
	if (retval) {
	...
	if (header.type >= 0) {
		...
		if (endpoint->local_type_id(header.type) >= 0) {
			if (do_callbacks_for(endpoint->local_type_id(header.type),
					endpoint->local_sender_id(header.sender), header.msg_time,
					header.payload_len, header.buffer)) {
				return -1;
			}
		}
...
void vrpn_File_Connection::find_superlative_user_times() {
	...
	if (retval == false) {
		...
		fprintf( stderr,
				"vrpn_File_Connection::find_superlative_user_times messed up the location in the file stream.\n");
		reset();
		return;
	}
	...
	} else // !preload and !accumulate
	{
		...
		if (d_currentLogEntry == NULL) // at the end of the file
		{
			...
			if (d_bookmark.oldCurrentLogEntryCopy == NULL) {
				...
				if (d_bookmark.oldCurrentLogEntryCopy == NULL) {
					fprintf( stderr,
							"Out of memory error:  vrpn_File_Connection::store_stream_bookmark\n");
...
bool vrpn_File_Connection::return_to_bookmark() {
	...
	} else // !preload and !accumulate
	{
		if (d_bookmark.oldCurrentLogEntryCopy == NULL) {
			...
			if (newBuffer == NULL) { // make sure we can allocate the memory before we do anything else
				return false;
			}
			d_time = d_bookmark.oldTime;
			retval |= fseek(d_file, d_bookmark.file_pos, SEEK_SET);
