...
static const unsigned char BIT_MASK = PORT_ERROR | PORT_SLCT | PORT_PE
		| PORT_ACK | PORT_BUSY;
#endif
...
static int VRPN_CALLBACK client_msg_handler(void *userdata,
		vrpn_HANDLERPARAM p);
...
vrpn_Button::vrpn_Button(const char *name, vrpn_Connection *c) :
		vrpn_BaseClass(name, c), num_buttons(0) {
	vrpn_BaseClass::init();
...
int vrpn_Button::register_types(void) {
	...
	change_message_id = d_connection->register_message_type(
			"vrpn_Button Change");

	//used to handle button states reports
	states_message_id = d_connection->register_message_type(
			"vrpn_Button States");
...
void vrpn_Button_Filter::set_momentary(vrpn_int32 which_button) {
	if (which_button >= num_buttons) {
		char msg[200];
		sprintf(msg,
				"vrpn_Button::set_momentary() buttons id %d is greater than the number of buttons(%d)\n",
...
void vrpn_Button_Filter::set_toggle(vrpn_int32 which_button,
	...
	if (which_button >= num_buttons) {
		char msg[200];
		sprintf(msg,
				"vrpn_Button::set_toggle() buttons id %d is greater then the number of buttons(%d)\n",
				which_button, num_buttons);
		send_text_message(msg, timestamp, vrpn_TEXT_ERROR);
		return;
	}
...
vrpn_Button_Example_Server::vrpn_Button_Example_Server(const char *name,
		vrpn_Connection *c, int numbuttons, vrpn_float64 rate) :
		vrpn_Button_Filter(name, c) {
	if (numbuttons > vrpn_BUTTON_MAX_BUTTONS) {
		num_buttons = vrpn_BUTTON_MAX_BUTTONS;
	} else {
...
vrpn_Button_PinchGlove::vrpn_Button_PinchGlove(const char* name,
		vrpn_Connection *c, const char *port, long baud) :
...
vrpn_Button_Remote::vrpn_Button_Remote(const char *name, vrpn_Connection *cn) :
	...
	if (d_connection != NULL) {
		if (register_autodeleted_handler(change_message_id,
				handle_change_message, this, d_sender_id)) {
			fprintf(stderr,
					"vrpn_Button_Remote: can't register change handler\n");
			d_connection = NULL;
		}
		if (register_autodeleted_handler(states_message_id,
				handle_states_message, this, d_sender_id)) {
			fprintf(stderr,
