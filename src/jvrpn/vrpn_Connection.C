...
vrpn_int32 vrpn_TranslationTable::addRemoteEntry(cName name,
		vrpn_int32 remote_id, vrpn_int32 local_id) {
	vrpn_int32 useEntry;
...
vrpn_Log::vrpn_Log(vrpn_TranslationTable * senders,
		...
		d_logFileName(NULL), d_logmode(vrpn_LOG_NONE), d_logTail(NULL), d_firstEntry(
		NULL), d_file(NULL), d_magicCookie(NULL), d_wroteMagicCookie(
				vrpn_FALSE), d_filters(NULL), d_senders(senders), d_types(types) {
...
int vrpn_Log::logIncomingMessage(vrpn_int32 payloadLen, struct timeval time,
...
//fprintf(stderr, "Logging incoming message of type %d.\n", type);
		return logMessage(payloadLen, time, type, sender, buffer, vrpn_TRUE);
	}
...
public:
	...
	int doSystemCallbacksFor(vrpn_int32 type, vrpn_int32 sender, timeval time,
			vrpn_uint32 len, const char * buffer, void * userdata);
...
vrpn_TypeDispatcher::vrpn_TypeDispatcher(void) :
		d_numTypes(0), d_numSenders(0), d_genericCallbacks(NULL) {
	int i;
...
int vrpn_TypeDispatcher::addHandler(vrpn_int32 type,
		vrpn_MESSAGEHANDLER handler, void * userdata, vrpn_int32 sender) {
...
#ifdef  VERBOSE
fprintf(stderr, "Adding user handler for type %d, sender %d\n", type, sender);
#endif
...
int vrpn_TypeDispatcher::doCallbacksFor(vrpn_int32 type, vrpn_int32 sender,
		timeval time, vrpn_uint32 len, const char * buffer) {
...
int vrpn_TypeDispatcher::doSystemCallbacksFor(vrpn_int32 type,
		vrpn_int32 sender, timeval time, vrpn_uint32 len, const char * buffer,
		void * userdata) {
	vrpn_HANDLERPARAM p;
...
static int vrpn_getmyIP(char * myIPchar, unsigned maxlen, const char * NIC_IP =
NULL,
...
#else
int vrpn_noint_block_read_timeout(int infile, char buffer[], int length,
...
#endif
		fprintf(stderr,
				"  (This probably means that another application has the port open already)\n");
		return (SOCKET) -1;
	}
...
int vrpn_udp_request_lob_packet(const char * machine,// Name of the machine to call
...
static
int vrpn_poll_for_accept(SOCKET listen_sock, SOCKET *accept_sock,
		double timeout = 0.0) {
	fd_set rfds;
	struct timeval t;
...
vrpn_Endpoint::vrpn_Endpoint(vrpn_TypeDispatcher * dispatcher,
		...
		status(BROKEN), d_remoteLogMode(0), d_remoteInLogName(NULL), d_remoteOutLogName(
		NULL), d_inLog(NULL), d_outLog(NULL), d_senders(NULL), d_types(
		NULL), d_dispatcher(dispatcher), d_connectionCounter(
				connectedEndpointCounter) {
	vrpn_Endpoint::init();
}

vrpn_Endpoint_IP::vrpn_Endpoint_IP(vrpn_TypeDispatcher * dispatcher,
		vrpn_int32 * connectedEndpointCounter) :
		vrpn_Endpoint(dispatcher, connectedEndpointCounter), d_tcpSocket(
		INVALID_SOCKET), d_tcpListenSocket(INVALID_SOCKET), d_tcpListenPort(0), d_remote_machine_name(
				NULL), d_remote_port_number(0), d_tcp_only(vrpn_FALSE), d_udpOutboundSocket(
				INVALID_SOCKET), d_udpInboundSocket(
		INVALID_SOCKET), d_tcpOutbuf(new char[vrpn_CONNECTION_TCP_BUFLEN]), d_udpOutbuf(
				new char[vrpn_CONNECTION_UDP_BUFLEN]), d_tcpBuflen(
				d_tcpOutbuf ? vrpn_CONNECTION_TCP_BUFLEN : 0), d_udpBuflen(
				d_udpOutbuf ? vrpn_CONNECTION_UDP_BUFLEN : 0), d_tcpNumOut(0), d_udpNumOut(
				0), d_tcpSequenceNumber(0), d_udpSequenceNumber(0), d_tcpInbuf(
				(char *) d_tcpAlignedInbuf), d_udpInbuf(
				(char *) d_udpAlignedInbuf), d_NICaddress(NULL) {
...
int vrpn_Endpoint_IP::mainloop(timeval * timeout) {
	...
	case CONNECTED:
		...
		if (vrpn_noint_select(fd_max + 1, &readfds, NULL, &exceptfds, timeout)
				== -1) {
		...
		if ((d_udpInboundSocket != -1) && FD_ISSET(d_udpInboundSocket, &readfds)) {
			...
			fprintf(stderr, "udp message read = %d\n", udp_messages_read);
#else
		...
		if (FD_ISSET(d_tcpSocket, &readfds)) {
			...
			fprintf(stderr,
				...
				fprintf(stderr, "tcp_message_read %d bytes\n", tcp_messages_read);
				}
			}
#else
...
#define      HISTSIZE 25
		{
			...
			fprintf(stderr, "\nHisto (tcp): ");
				...
				fprintf(stderr, "%d ",tcp_histogram[i]);
					tcp_histogram[i] = 0;
				}
			fprintf(stderr, "\n");
			fprintf(stderr, "      (udp): ");
				for (i = 0; i < HISTSIZE+1; i++) {
				fprintf(stderr, "%d ",udp_histogram[i]);
					udp_histogram[i] = 0;
				}
			fprintf(stderr, "\n");
			}
	...
	fprintf(stderr, "TRYING_TO_CONNECT\n");
		...
		if (d_tcp_only) {
			if (time_to_try_again) {
				...
				if (connect_tcp_to(d_remote_machine_name, d_remote_port_number)
						== 0) {
		...
		if (ret == 1) {	// Got one!
			status = COOKIE_PENDING;
#ifdef	VERBOSE
		fprintf(stderr, "vrpn: Connection established\n");
#endif
...
//fprintf(stderr, "BROKEN - vrpn_Endpoint::mainloop.\n");
		...
		if (time_to_try_again) {
			if (vrpn_udp_request_lob_packet(d_remote_machine_name,
					d_remote_port_number, d_tcpListenPort, d_NICaddress)
					== -1) {
				fprintf(stderr,
...
int vrpn_Endpoint_IP::pack_message(vrpn_uint32 len, timeval time,
	...
	if ((d_udpOutboundSocket == -1)
		...
		if (d_tcpSocket == -1) {
			...
			ret = tryToMarshall(d_tcpOutbuf, d_tcpBuflen, d_tcpNumOut, len,
					time, type, sender, buffer, d_tcpSequenceNumber);
			d_tcpNumOut += ret;
			if (ret > 0) {
				d_tcpSequenceNumber++;
			}
...
int vrpn_Endpoint::pack_log_description(void) {
	...
	size_t bufsize = 2 * sizeof(vrpn_int32) + strlen(inName) + 1
			+ strlen(outName) + 1;
	...
	int ret = pack_message(static_cast<vrpn_uint32>(bufsize - bufleft), now,
			vrpn_CONNECTION_LOG_DESCRIPTION, d_remoteLogMode, buf,
			vrpn_CONNECTION_RELIABLE);
	delete[] buf;
	return ret;
}
...
#ifdef  VERBOSE2
fprintf(stderr, "vrpn_Endpoint::handle_tcp_messages() called\n");
#endif
...
#ifdef  VERBOSE2
fprintf(stderr, "vrpn_Endpoint::handle_udp_messages() called\n");
#endif
	...
	do {
		...
		if (FD_ISSET(d_udpInboundSocket, &exceptfds)) {
			fprintf(stderr,
					"vrpn: vrpn_Endpoint::handle_udp_messages: Exception on socket\n");
			return (-1);
		}
...
void vrpn_Endpoint_IP::poll_for_cookie(const timeval * pTimeout) {
	...
	if (FD_ISSET(d_tcpSocket, &readfds)) {
		...
		fprintf(stderr, "vrpn_Endpoint::poll_for_cookie() got cookie\n");
		}
#endif
	}
}
...
int vrpn_Endpoint_IP::finish_new_connection_setup(void) {
	...
	if (recvbuf == NULL) {
		fprintf(stderr,
				"vrpn_Endpoint_IP::finish_new_connection_setup(): Out of memory when allocating receiver buffer\n");
		status = BROKEN;
		return -1;
	}
...
fprintf(stderr, "Message length is %d (buffer length %lu).\n", len, inbuf_len);
#endif
	...
	if (type >= 0) {        // User handler, map to local id
		...
		if (local_type_id(type) >= 0) {
			if (d_dispatcher->doCallbacksFor(local_type_id(type),
					local_sender_id(sender), time, payload_len, bufptr)) {
				return -1;
			}
		}
...
int vrpn_Endpoint::tryToMarshall(char * outbuf, vrpn_int32 &buflen,
		vrpn_int32 &numOut, vrpn_uint32 len, timeval time, vrpn_int32 type,
		vrpn_int32 sender, const char * buffer, vrpn_uint32 sequenceNumber) {
	int retval;
...
int vrpn_Endpoint::marshall_message(char * outbuf, // Base pointer to the output buffer
...
//type, sender, len);
	...
	*(vrpn_uint32*) (void*) (&outbuf[curr_out]) = htonl(seqNo);
	...
	curr_out += ceil_len;
#ifdef  VERBOSE
fprintf(stderr, "Marshalled: len %d, ceil_len %d: '", len, ceil_len);
fprintf(stderr, "'\n");
#endif
...
#ifdef	VERBOSE
fprintf(stderr, "Registering other-side type: '%s'\n", type_name);
#endif
	...
	if (local_id == -1) {
		...
		fprintf(stderr, "vrpn_Endpoint::handle_type_message:  NULL d_parent "
...
int vrpn_Endpoint::handle_sender_message(void *userdata, vrpn_HANDLERPARAM p) {
	...
	if (static_cast<size_t>(p.payload_len) > sizeof(cName)) {
		fprintf(stderr,
				"vrpn: vrpn_Endpoint::handle_sender_message():Sender name too long\n");
		return -1;
	}
...
#ifdef	VERBOSE
fprintf(stderr, "Registering other-side sender: '%s'\n", sender_name);
#endif
	...
	if (local_id == -1) {
		...
		fprintf(stderr, "vrpn_Endpoint::handle_sender_message:  NULL d_parent "
...
int vrpn_Endpoint::pack_type_description(vrpn_int32 which) {
	...
	vrpn_uint32 len = static_cast<vrpn_int32>(strlen(
...
#ifdef	VERBOSE
fprintf(stderr, "  vrpn_Connection: Packing type '%s', %d\n",
...
#ifdef	VERBOSE
fprintf(stderr, "  vrpn_Connection: Packing sender '%s'\n",
...
int vrpn_Connection::pack_message(vrpn_uint32 len, struct timeval time,
	...
	fprintf(stderr, "vrpn_Connection::pack_message: bad type (%d)\n", type);
		return -1;
	}
	...
	if (type >= 0) {
		...
		fprintf(stderr, "vrpn_Connection::pack_message: bad sender (%d)\n", sender);
			return -1;
		}
	}
	...
	for (i = 0; i < d_numEndpoints; i++) {
		if (d_endpoints[i]
				&& (d_endpoints[i]->pack_message(len, time, type, sender,
						buffer, class_of_service) != 0)) {
			ret = -1;
		}
	}
...
vrpn_Connection::vrpn_Connection(const char * local_in_logfile_name,
		...
		d_numEndpoints(0), d_numConnectedEndpoints(0), d_references(0), d_autoDeleteStatus(
				false), d_dispatcher(NULL), d_serverLogCount(0), d_serverLogMode(
				(local_in_logfile_name ? vrpn_LOG_INCOMING : vrpn_LOG_NONE)
						| (local_out_logfile_name ?
								vrpn_LOG_OUTGOING : vrpn_LOG_NONE)), d_serverLogName(
		NULL), d_endpointAllocator(epa), d_updateEndpoint(vrpn_FALSE) {
	int retval;
...
vrpn_Connection::vrpn_Connection(const char * local_in_logfile_name,
		...
		d_numEndpoints(0), d_numConnectedEndpoints(0), d_references(0), d_autoDeleteStatus(
				false), d_dispatcher(NULL), d_serverLogCount(0), d_serverLogMode(
				vrpn_LOG_NONE), d_serverLogName(NULL), d_endpointAllocator(epa), d_updateEndpoint(
				vrpn_FALSE) {
	...
	endpoint->d_remoteLogMode =
			(((remote_in_logfile_name && strlen(remote_in_logfile_name) > 0) ?
					vrpn_LOG_INCOMING : vrpn_LOG_NONE)
					| ((remote_out_logfile_name
							&& strlen(remote_out_logfile_name) > 0) ?
							vrpn_LOG_OUTGOING : vrpn_LOG_NONE));
	if (!remote_in_logfile_name) {
		endpoint->d_remoteInLogName = new char[10];
		strcpy(endpoint->d_remoteInLogName, "");
	} else {
		endpoint->d_remoteInLogName = new char[strlen(remote_in_logfile_name)
				+ 1];
	...
	if (!remote_out_logfile_name) {
		...
		endpoint->d_remoteOutLogName = new char[strlen(remote_out_logfile_name)
				+ 1];
...
void vrpn_Connection::get_log_names(char **local_in_logname,
		char **local_out_logname, char **remote_in_logname,
	...
	if (remote_in_logname != NULL) {
		if (endpoint->d_remoteInLogName != NULL) {
			*remote_in_logname = new char[strlen(endpoint->d_remoteInLogName)
					+ 1];
	...
	if (remote_out_logname != NULL) {
		if (endpoint->d_remoteOutLogName != NULL) {
			*remote_out_logname = new char[strlen(endpoint->d_remoteOutLogName)
					+ 1];
...
#ifdef	VERBOSE
fprintf(stderr, "Just read disconnect message from logfile\n");
#endif
...
vrpn_Connection * vrpn_get_connection_by_name(const char * cname,
	...
	if (!c) {
		...
		if (is_file) {
			...
			c = new vrpn_Connection_IP(cname, port, local_in_logfile_name,
					local_out_logfile_name, remote_in_logfile_name,
					remote_out_logfile_name, NIC_IPaddress);
		}
...
vrpn_Connection * vrpn_create_server_connection(const char * cname,
...
#else
		fprintf(stderr,
				"vrpn_create_server_connection(): MPI support not compiled in.  Set VRPN_USE_MPI in vrpn_Configure.h and recompile.\n");
		delete[] location;
		return NULL;
#endif
	} else {
	...
	if (!c) {		// creation failed
		fprintf(stderr,
				"vrpn_create_server_connection(): Could not create new connection.");
		return NULL;
	}
...
#ifdef	VERBOSE
fprintf(stderr, "  Received request for UDP channel to %s\n", p.buffer);
#endif
...
//fprintf(stderr, "BROKEN - vrpn_Connection::server_check_for_incoming_connections.\n");
		...
		sprintf(fromname, "%lu.%lu.%lu.%lu", (addr_num) >> 24,
				...
				addr_num & 0xff);
	fprintf(stderr, "vrpn: Connection request received from %s: %s\n", fromname,
				msg);
	...
	fprintf(stderr, "vrpn: TCP connection request received.\n");
...
vrpn_Connection_IP::vrpn_Connection_IP(unsigned short listen_port_no,
		const char * local_in_logfile_name, const char * local_out_logfile_name,
		const char * NIC_IPaddress,
		vrpn_Endpoint_IP * (*epa)(vrpn_Connection *, vrpn_int32 *)) :
		vrpn_Connection(local_in_logfile_name, local_out_logfile_name, epa), listen_udp_sock(
		INVALID_SOCKET), listen_tcp_sock(INVALID_SOCKET), d_NIC_IP(NULL) {
...
#ifdef	VERBOSE
	fprintf(stderr, "vrpn: Listening for requests on port %d\n", listen_port_no);
#endif
	}
...
vrpn_Connection_IP::vrpn_Connection_IP(const char * station_name, int port,
		const char * local_in_logfile_name, const char * local_out_logfile_name,
		const char * remote_in_logfile_name,
		const char * remote_out_logfile_name, const char * NIC_IPaddress,
		vrpn_Endpoint_IP * (*epa)(vrpn_Connection *, vrpn_int32 *)) :
		vrpn_Connection(local_in_logfile_name, local_out_logfile_name,
				remote_in_logfile_name, remote_out_logfile_name, epa), listen_udp_sock(
		INVALID_SOCKET), listen_tcp_sock(INVALID_SOCKET), d_NIC_IP(NULL) {
...
#ifdef	VERBOSE
	fprintf(stderr, "vrpn_Connection_IP: Getting the TCP port to listen on\n");
#endif
...
#ifdef	VERBOSE
		fprintf(stderr, "vrpn: Connection established on initial try\n");
#endif
			// Set up the things that need to happen when a new connection
			// is established.
			if (endpoint->setup_new_connection()) {
...
//fprintf(stderr, "BROKEN - vrpn_Connection_IP::vrpn_Connection_IP.\n");
	...
	if (istcp) {
		...
		if (!endpoint->d_remote_machine_name) {
			fprintf(stderr,
					"vrpn_Connection_IP: Can't get remote machine name for tcp: connection!\n");
			connectionStatus = BROKEN;
			return;
		}
		endpoint->d_remote_port_number = port;
...
#ifdef	VERBOSE
	fprintf(stderr, "vrpn_Connection_IP: Getting the TCP port to connect with.\n");
#endif

		// Set up the connection that we will connect with.
		// Blocks, doesn't it?
		retval = endpoint->connect_tcp_to(endpoint->d_remote_machine_name,
				port);
