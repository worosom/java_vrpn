...
vrpn_Analog::vrpn_Analog(const char * name, vrpn_Connection * c) :
		vrpn_BaseClass(name, c), num_channel(0) {
...
#endif
	if (d_connection
			&& d_connection->pack_message(len, timestamp, channel_m_id,
...
vrpn_Serial_Analog::vrpn_Serial_Analog(const char * name, vrpn_Connection * c,
		const char * port, int baud, int bits, vrpn_SER_PARITY parity,
		bool rts_flow) :
		vrpn_Analog(name, c), serial_fd(-1), baudrate(0), bufcounter(0) {
	// Initialize
	portname[0] = '\0';
	buffer[0] = '\0';
...
vrpn_int32 vrpn_Analog_Server::setNumChannels(vrpn_int32 sizeRequested) {
	if (sizeRequested < 0)
		sizeRequested = 0;
	if (sizeRequested > vrpn_CHANNEL_MAX)
		sizeRequested = vrpn_CHANNEL_MAX;
	num_channel = sizeRequested;
	return num_channel;
}

vrpn_Clipping_Analog_Server::vrpn_Clipping_Analog_Server(const char *name,
		vrpn_Connection *c, vrpn_int32 numChannels) :
...
int vrpn_Clipping_Analog_Server::setClipValues(int chan, double min,
		double lowzero, double highzero, double max) {
	...
	if ((lowzero < min) || (highzero < lowzero) || (max < highzero)) {
		fprintf(stderr,
				"vrpn_Clipping_Analog_Server::setClipValues: Out of order mapping\n");
		return -1;
	}
...
int vrpn_Clipping_Analog_Server::setChannelValue(int chan, double value) {
	...
	if ((value >= clips.lower_zero) && (value <= clips.upper_zero)) {
		...
		channel[chan] = (value - clips.upper_zero)
				/ (clips.maximum_val - clips.upper_zero);
	} else {
		...
		channel[chan] = (value - clips.lower_zero)
				/ (clips.lower_zero - clips.minimum_val);
	}
