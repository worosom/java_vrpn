...
/**
 * Represents the connection between the Java user-interface code and the C++
 * VRPN code
 * 
 * @author Eric Boren
 * 
...
public class JniLayer {
	static {
		try {
			System.out.println("JNI - Trying to load native vrpn lib");
			System.loadLibrary("jvrpn");
		} catch (UnsatisfiedLinkError ule) {
			System.err
					.println("JNI - WARNING: Could not load native vrpn lib (libjvrpn)"
							+ "\n" + ule.getMessage());
		}
	}
	...
	/**
	 * Instantiates a new JniLayer, for communication between this Android app
	 * and the VRPN network. Should only be called from JniBuilder.
	 * 
	 * @param analogs
	 *            - an array representing the number of channels in each analog
	 *            server
	 * @param numButtons
	 *            - the number of buttons used by the app
	 * @param port
	...
	public void start() {
		if (!this.running) {
			this.cppPointer = jni_layer_initialize(this.analogs,
					this.numButtons, this.port);
			System.out.println("Created JniLayer at address: "
					+ this.cppPointer);
	...
	/**
	 * Deconstructs all objects on the C++ side and closes the connection. This
	 * is necessary any time the app loses focus.
	 */
	public void stop() {
		if (this.running) {
	...
	/**
	 * Forces a data report to be sent from the given analog server, even if the
	 * value has not changed.
	 * 
	 * @param analogId
	 *            - The identifier of the analog server for which to report a
	 *            value
	 */
	 ...
	 */
	private native long jni_layer_initialize(int[] numAnalogChannels,
			int numButtons, int port);

	private native void jni_layer_mainloop(long pointer);

	private native void jni_layer_set_button(long pointer, int buttonId,
			int state);

	private native void jni_layer_set_analog(long pointer, int analogId,
			int channel, float val);
